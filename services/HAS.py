...
class HomeAssistantService:
	...
	def __init__(self, language, hardware):
		...
		#################################################
		############# Debug
		self.things["devices"]["ventilador"] = IoTDevice.from_json('{"alias": "ventilador", "address": "192.168.1.50", "mac": "FF:FF:FF:00:00:00", "category": "Temperature", "properties": {"status": "off"}, "intents": [ { "turn on": { "request": "http://{ipaddress}/?relay={status}}", "parameters": { "ipaddress": "255.255.255.255", "status": "on" } } }, { "turn off": { "request": "http://{ipaddress}/?relay={status}", "parameters": { "ipaddress": "255.255.255.255", "status": "off" } } } ]}') # Switch("192.168.1.50", "FF:FF:FF:00:00:00", "ventilador")
		self.things["devices"]["bombilla"] = IoTDevice.from_json('{"alias": "bombilla", "address": "192.168.1.51", "mac": "FF:FF:FF:00:00:01", "category": "Light", "properties": {"status": "off", "color": "#000000", "intensity": 0}, "intents": [ { "turn on": { "request": "http://{ipaddress}/?relay={status}}", "parameters": { "ipaddress": "255.255.255.255", "status": "on" } } }, { "turn off": { "request": "http://{ipaddress}/?relay={status}", "parameters": { "ipaddress": "255.255.255.255", "status": "off" } } }, { "color set": { "request": "http://{address}/?color={color}", "parameters": { "address": "255.255.255.255", "color": "#000000" } } }, { "intensity set": { "request": "http://{address}/?intensity={intensity}", "parameters": { "address": "255.255.255.255", "intensity": 0 } } } ]}')# Bolb("192.168.1.51", "AF:AF:AF:00:00:00", "bombilla")
